<template>
	<div class="page">
		<div v-if="!$store.state.canvas.data.locked" class="tools">
...
export default {
	...
	methods: {
		async open() {
			...
			const data = await this.$axios.$get(`http://localhost:8080/myleave/fileView.do?MYLEAVE_ID=${this.$route.query.id}`);
			if (data && data.id) {
